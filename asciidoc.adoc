...
For example, the placeholder `pass:[<var>PROJECT_ID</var>]` represents
a project ID in sample code, commands, and example output.

In the following example, `pass:[<var>GROUP</var>]` is required,
but `pass:[<var>GLOBAL_FLAG</var>]` and `pass:[<var>FILENAME</var>]` are optional:

[source,sh,subs="+macros"]
----
gcloud dns pass:[<var>GROUP</var>] pass:[[<var>GLOBAL_FLAG</var>]] pass:[[<var>FILENAME</var>]]
----
...
[literal,subs="+macros"]
ssh-rsa +++<var>KEY_VALUE</var>+++ +++<var>USERNAME</var>+++
